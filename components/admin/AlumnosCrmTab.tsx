  ...
  const students = useMemo(() => {
    ...
    Object.entries(clinicalProfiles).forEach(([phone, profile]) => {
      ...
      if (!map.has(key)) {
        map.set(key, {
          phone,
          name: profile.studentName || (profile.conditionReason ? `Alumno (${phone})` : phone),
          bookings: [],
          lastDate: new Date().toISOString().split("T")[0],
          totalSpent: 0,
          hasActivePack: false,
        });
      } else if (profile.studentName && (!map.get(key)!.name || map.get(key)!.name === phone)) {
        ...
        <div className="lg:col-span-7">
          {currentStudent ? (
            <div className="p-6 rounded-2xl bg-surface border border-border space-y-6 shadow-sm">
              ...
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-4 border-b border-border">
                ...
                <div className="flex items-center gap-2 flex-wrap">
                  {onScheduleBookingForStudent && (
                    <button
                      onClick={() => onScheduleBookingForStudent(currentStudent.name, currentStudent.phone)}
              ...
              <div className="space-y-2">
                ...
                <textarea
                  ...
                  onChange={(e) => handleUpdateProfileField("medicalNotes", e.target.value)}
              ...
              <form onSubmit={handleAddEvolutionLog} className="p-4 rounded-xl bg-surface-raised border border-border space-y-3">
                <h4 className="text-xs font-condensed font-bold uppercase tracking-wider text-foreground">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-4 gap-3">
                  <div>
                    <label className="block text-[11px] font-condensed uppercase text-muted mb-1">Fecha</label>
                    <input
                      type="date"
                      value={newLogDate}
                  ...
                  <div>
                    <label className="block text-[11px] font-condensed uppercase text-muted mb-1">Dolor Pre ({newLogPainBefore})</label>
                    <input
                      type="range"
                      min={0}
                      max={10}
                  ...
                  <div>
                    <label className="block text-[11px] font-condensed uppercase text-muted mb-1">Dolor Post ({newLogPainAfter})</label>
                    <input
                      type="range"
                      min={0}
                      max={10}
                  ...
                  <div>
                    <label className="block text-[11px] font-condensed uppercase text-muted mb-1">Tensión Fascial</label>
                    <select
                      value={newLogTension}
                ...
                <div>
                  <label className="block text-[11px] font-condensed uppercase text-muted mb-1">Observaciones / Ejercicios Realizados</label>
                  <textarea
                    rows={2}
                    required
                    value={newLogNotes}
                  ...
                  <div className="space-y-2 max-h-60 overflow-y-auto pr-1">
                    {currentProfile.evolutionLogs.map((log, idx) => (
                      ...
                      >
                        ...
                        <p className="text-foreground/90 font-sans">{log.notes}</p>
                      </div>
                    ))}
                  </div>
                ) : (
            ...
            <div className="text-center py-20 p-8 rounded-2xl bg-surface border border-dashed border-border text-muted">
              <svg className="w-10 h-10 mx-auto mb-2 text-muted/40" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" />
              </svg>
              <p className="text-sm font-condensed font-bold uppercase text-foreground">Seleccioná un alumno de la lista</p>
              <p className="text-xs mt-1 font-sans">Podrás registrar su evolución de dolor EVA, bitácoras y ficha PDF.</p>
            </div>
          )}
        </div>
      </div>
