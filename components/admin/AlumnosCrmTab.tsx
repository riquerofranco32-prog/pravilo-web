  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-4">
        ...
        <div className="p-4 rounded-2xl bg-white/[0.02] border border-white/[0.06]">
          <span className="text-[11px] font-mono uppercase text-amber-400 block">Para Reactivación</span>
        ...
        <div className="lg:col-span-5 space-y-4">
          ...
          <div className="space-y-2 max-h-[680px] overflow-y-auto pr-1">
            {filteredStudents.length === 0 ? (
              <p className="text-xs text-white/40 text-center py-8">No se encontraron alumnos.</p>
            ) : (
                ...
                return (
                  ...
                  >
                    <div className="flex items-start justify-between gap-2">
                      <div>
                        <h4 className="text-sm font-semibold text-white flex items-center gap-1.5">
                          {s.name}
                          {s.hasActivePack && (
                            <span className="w-2 h-2 rounded-full bg-purple-400" title="Pack Activo" />
                          )}
                        </h4>
                        <p className="text-xs text-white/50 font-mono">{s.phone || "Sin teléfono"}</p>
                      </div>
                    ...
                    {profile?.conditionReason && (
                      <p className="mt-2 text-xs text-amber-300/80 truncate font-mono">
                        🩺 {profile.conditionReason}
                      </p>
                    )}
            ...
            <div className="p-6 rounded-2xl bg-white/[0.02] border border-white/[0.08] space-y-6">
              ...
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-4 border-b border-white/[0.08]">
                ...
                <div className="flex items-center gap-2">
                  <button
                    ...
                    className="px-3 py-1.5 rounded-xl bg-white/[0.04] hover:bg-white/[0.08] border border-white/[0.1] text-xs text-white/80 flex items-center gap-1.5 transition-all"
                  >
                  ...
                  {currentStudent.phone && (
                    <a
                      ...
                      className="px-3 py-1.5 rounded-xl bg-emerald-500/15 hover:bg-emerald-500/25 border border-emerald-500/30 text-emerald-300 text-xs font-semibold flex items-center gap-1.5 transition-all"
                    >
                      Reactivar WA
                    </a>
                  )}
              ...
              <div className="space-y-2">
                ...
                <input
                  ...
                  className="w-full px-3.5 py-2.5 rounded-xl bg-white/[0.04] border border-white/[0.1] text-xs sm:text-sm text-white placeholder-white/30 focus:border-amber-500 focus:outline-none"
                />
              </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    <div className="flex justify-between text-xs mb-1">
                      <span className="text-white/60">Dolor Inicial (Primera Sesión):</span>
                  ...
                  <div>
                    <div className="flex justify-between text-xs mb-1">
                      <span className="text-white/60">Dolor Actual:</span>
              ...
              <form
                ...
                className="p-4 rounded-xl bg-amber-500/[0.03] border border-amber-500/20 space-y-3"
              >
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                  <div>
                    <label className="block text-[10px] text-white/50 mb-0.5">Fecha</label>
                    <input
                      type="date"
                      value={newLogDate}
                      onChange={(e) => setNewLogDate(e.target.value)}
                      className="w-full px-2 py-1 rounded bg-black/40 border border-white/10 text-xs font-mono text-white"
                    />
                  </div>
                  <div>
                    <label className="block text-[10px] text-white/50 mb-0.5">Dolor Pre (0-10)</label>
                    <input
                      type="number"
                      min={0}
                      max={10}
                      ...
                      className="w-full px-2 py-1 rounded bg-black/40 border border-white/10 text-xs font-mono text-rose-400"
                    />
                  </div>
                  <div>
                    <label className="block text-[10px] text-white/50 mb-0.5">Dolor Post (0-10)</label>
                    <input
                      type="number"
                      min={0}
                      max={10}
                      ...
                      className="w-full px-2 py-1 rounded bg-black/40 border border-white/10 text-xs font-mono text-emerald-400"
                    />
                  </div>
                  <div>
                    <label className="block text-[10px] text-white/50 mb-0.5">Tensión</label>
                    <select
                      value={newLogTension}
                      onChange={(e) => setNewLogTension(e.target.value as any)}
                      className="w-full px-2 py-1 rounded bg-[#18191c] border border-white/10 text-xs text-white"
                    >
                ...
                <div>
                  <textarea
                    ...
                    className="w-full px-3 py-2 rounded-xl bg-black/40 border border-white/10 text-xs text-white placeholder-white/30 focus:outline-none focus:border-amber-500"
                  />
                </div>
                ...
                <div className="flex justify-end">
                  <button
                    type="submit"
                    className="px-4 py-1.5 rounded-lg bg-amber-500 hover:bg-amber-400 text-black font-semibold text-xs shadow transition-all"
                  >
