...
export function GaleriaTab({
  ...
  const priorityCount = images.filter(
    (img) => img.priority && img.visible !== false,
  ).length;
  ...
  const handleDelete = (id: string) => {
    if (!window.confirm("¿Seguro que querés eliminar esta foto de la galería?"))
      return;
  ...
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!file.type.startsWith("image/")) {
      alert(
        "Por favor seleccioná un archivo de imagen válido (JPG, PNG, WEBP).",
      );
      return;
    }
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 p-6 rounded-3xl bg-surface-raised border border-border">
        <div>
          <div className="flex items-center gap-2.5">
            <div className="w-10 h-10 rounded-xl bg-accent/15 border border-accent/30 text-accent-text flex items-center justify-center">
              <svg
                className="w-5 h-5"
                fill="none"
                viewBox="0 0 24 24"
            ...
            <div>
              ...
              <p className="text-xs text-muted font-sans">
                Agregá, ordená o desactivá las fotos que se muestran en el
          ...
          <div className="flex items-center gap-3 mt-3 text-xs font-condensed font-bold uppercase tracking-wider">
            <span className="px-3 py-1 rounded-full bg-surface border border-border text-foreground">
              Total:{" "}
              <strong className="text-accent-text">{images.length}</strong>{" "}
              fotos
            </span>
            ...
            <span className="px-3 py-1 rounded-full bg-amber-500/10 border border-amber-500/30 text-amber-400 flex items-center gap-1">
              <span>
          ...
          return (
            ...
            >
              ...
              <div className="relative aspect-3/4 w-full bg-surface overflow-hidden group">
                ...
                <div className="absolute top-2.5 inset-x-2.5 flex items-center justify-between pointer-events-none">
                  ...
                  <div className="flex items-center gap-1.5">
                    {isPriority && (
                      <span className="px-2 py-0.5 rounded-full bg-amber-400 text-black text-[10px] font-condensed font-black uppercase tracking-wider shadow-md flex items-center gap-1">
                        <svg
              ...
              <div className="p-4 space-y-3 flex-1 flex flex-col justify-between">
                <div>
                  <p
                    className="text-xs font-semibold text-foreground font-sans line-clamp-2"
                    title={img.alt}
                  >
                    {img.alt}
                  </p>
                  <p
                    className="text-[10px] text-muted/60 truncate font-mono mt-1"
                    title={img.src}
                  >
                    {img.src.startsWith("data:")
                      ? "Foto subida desde dispositivo"
                      : img.src}
                  </p>
                </div>
                ...
                <div className="space-y-2 pt-2 border-t border-border">
                  ...
                  <div className="grid grid-cols-2 gap-1.5">
                    <button
                      ...
                      className={`py-1.5 px-2 rounded-lg text-[11px] font-condensed font-bold uppercase tracking-wider border transition-all flex items-center justify-center gap-1.5 ${
                        isPriority
                    ...
                    >
                      <svg
                        className={`w-3 h-3 ${isPriority ? "fill-amber-300 text-amber-300" : "fill-none text-muted"}`}
                        viewBox="0 0 24 24"
                        stroke="currentColor"
                      >
                        <path
                    ...
                    <button
                      ...
                      title={
                        isVisible ? "Ocultar de la web" : "Mostrar en la web"
                      }
                    >
                      {isVisible ? (
                        <>
        ...
        >
          ...
          >
            <div className="flex items-center justify-between border-b border-border pb-4">
              <div className="flex items-center gap-2.5">
                <div className="w-8 h-8 rounded-lg bg-accent/15 border border-accent/30 text-accent-text flex items-center justify-center">
                  <svg
                    className="w-4 h-4"
                    fill="none"
                    viewBox="0 0 24 24"
              ...
              <div>
                ...
                >
                  ...
                  <div className="space-y-1">
                    <div className="w-10 h-10 mx-auto rounded-xl bg-surface border border-border flex items-center justify-center text-muted group-hover:border-accent/40 group-hover:text-accent-text group-hover:scale-110 transition-all">
                      <svg
                        className="w-5 h-5"
                        fill="none"
                        viewBox="0 0 24 24"
